...
void UAuraAbilitySystemComponent::ForEachAbility(const FForEachAbility& Delegate)
{
	...
	for (const FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		...
		{
			UE_LOG(LogAura, Error, TEXT("Failed to execute delegate to %hs"), __FUNCTION__);
		}
	}
}
...
void UAuraAbilitySystemComponent::UpgradeAttributes(const FGameplayTag& AttributeTag)
{
	if (GetAvatarActor()->Implements<UPlayerInterface>())
	{
		...
		{
			SeveralUpgradeAttributes(AttributeTag);
		}
	}
}

void UAuraAbilitySystemComponent::SeveralUpgradeAttributes_Implementation(const FGameplayTag& AttributeTag)
{
...
void UAuraAbilitySystemComponent::SeverEquipAbility_Implementation(const FGameplayTag& AbilityTag,
	const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			...
			//Cleat this ability's slot, just in case, it's a different slot.
